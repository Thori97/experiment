...
int main(int argc, char ** argv) {
  ...
  double f1 = atof(argv[2]), f2 = atof(argv[3]), omega = 2*M_PI/n;
  double omega1 = (2*M_PI) * f1, omega2 = (2*M_PI) * f2; 
  ...
  while (1) {
    ...
    int mm = 0,mmm=0;
    for(int i=0;i<n;i++){
        if(f2<(double)((44100*i)/n) || (double)((44100*i)/n)<f1){
            Y[i] = 0;
            mm++;
        }
        else{printf("%f, %f\n",creal(Y[i]),cimag(Y[i]));mmm++;}
    }
    printf("mmm = %d\n", mmm);
    printf("rate  = %f\n",(double)mm/n);
